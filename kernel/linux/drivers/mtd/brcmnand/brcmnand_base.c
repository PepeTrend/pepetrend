...
static int brcmnand_getChipSize(struct mtd_info *mtd)
{
	struct brcmnand_chip *chip = mtd->priv;
	return chip->mtdSize;
}

static unsigned int brcmnand_getChipBbtSize(struct mtd_info *mtd)
{
	struct brcmnand_chip *chip = mtd->priv;
	return chip->bbtSize;
}
...
// << [CTFN-SYS-024-1] jojopo : Support Flash Image Backup from GUI for NAND Flash , 2012/05/30
	mtd->getChipSize = brcmnand_getChipSize;
	mtd->getChipBbtSize = brcmnand_getChipBbtSize;
// >> [CTFN-SYS-024-1] end
