...
static int usb_stor_scan_thread(void * __us)
{
	...
	char 			*speed, *usb_ver;
	switch (us->pusb_dev->speed) {
		case USB_SPEED_LOW:	speed = "low"; usb_ver = "1.0"; break;
		case USB_SPEED_FULL:	speed = "full"; usb_ver = "1.1";	break;
		case USB_SPEED_HIGH:	speed = "high"; usb_ver = "2.0";	break;
		default: 		speed = "?"; usb_ver = "?";	break;
	}	
	printk(KERN_NOTICE
		"LITEUSB device %s %s, class 'USB %s (%s-speed) storage', plugged in\n", dev_driver_string(&us->pusb_dev->dev), dev_name(&us->pusb_dev->dev), usb_ver, speed);
	//>> Joba Yang End
...
printk("storage_probe1: result %d\n", result);
if(result == -ENODEV) {
	int p;
	if (us->unusual_dev->initFunction) {
		p = us->unusual_dev->initFunction(us);
...
BadDevice:
	US_DEBUGP("storage_probe() failed\n");
//<< [CTFN-3G-001] MHTsai: Support 3G feature, 2010/08/09
	printk("storage_probe2: result %d\n", result);
	if(result == -ENODEV) {
		int p;
		if (us->unusual_dev->initFunction) {
			p = us->unusual_dev->initFunction(us);
