...
//<< [CTFN-3G-001] Dylan : Fix remove 3G WAN service will cause call trace, 2010/10/25
#include <linux/ct_autoconf.h>
//>> [CTFN-3G-001] End
...
//<< [CTFN-3G-001] Dylan : Fix remove 3G WAN service will cause call trace, 2010/10/25
    ...
    if(netdev_path_next_dev(dev) == NULL) /* to avoid the next device is NULL */
	return 0;
#endif
//>> CTFN-3G-001] End
...
//<< [CTFN-SYS-037] Joba Yang Start
	if(skb->dev->name[0] == 'w' && skb->dev->name[1] == 'l' && skb->protocol == __constant_htons(ETH_P_CT)){
		kfree_skb(skb);
		ret = NET_RX_DROP;
		goto out;
	}
