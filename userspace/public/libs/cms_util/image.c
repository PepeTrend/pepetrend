...
static CmsRet flashImage(char *imagePtr, UINT32 imageLen, CmsImageFormat format, FW_UPGRADE_FROM from);
char FROM_STR[CMS_IMAGE_FROM_END][8] = 
{
	"HTTP",
	"FTP",
	"TFTP",
...
extern CmsRet devCtl_closeWatchDogTime();
//>> [CTFN-SYS-038] End
...
//<< Joba Yang [CTFN-MGMT-024] email function
CmsRet flashImage(char *imagePtr, UINT32 imageLen, CmsImageFormat format, FW_UPGRADE_FROM from)
//>> Joba Yang
{
	...
	//<< Joba Yang [CTFN-MGMT-024] email function
	//char command[128];
	//syslog(LOG_WARNING, "Firmware update completed for this device.");
	//sprintf(command, "echo \"Firmware update(%s)\" > /var/emailContent ; killall -16 email", FROM_STR[from]);
	//system(command);
	if (cmsPsp_set("FWUPDATE", "y", 1) != CMSRET_SUCCESS)
    {
        cmsLog_error("Could not save firmware update into scratch-pad!");
    }
	//Wait 3 second for send email to server for notification.
	//sleep(3);
	//>> Joba Yang End
...
UINT32 cmsImg_getFlashBlockCount(void)
{
   CmsRet ret;
   UINT32 size=0;

   ret = devCtl_boardIoctl(BOARD_IOCTL_GET_BLOCK_COUNT, 0, NULL, 0, 0, (void *)&size);
   if (ret != CMSRET_SUCCESS)
   {
...
UINT32 cmsImg_getFlashBlockSize(UINT32 BlockIndex)
{
   CmsRet ret;
   UINT32 size=0;

   ret = devCtl_boardIoctl(BOARD_IOCTL_GET_BLOCK_SIZE, 0, NULL, 0, BlockIndex, (void *)&size);
   if (ret != CMSRET_SUCCESS)
   {
      cmsLog_error("boardIoctl to get config flash size failed.");
      size = 0;
 ...
 */
//<< Joba Yang [CTFN-MGMT-024] email function
CmsRet cmsImg_writeImage(char *imagePtr, UINT32 imageLen, void *msgHandle, FW_UPGRADE_FROM from)
//>> Joba Yang End
{
...
//>> [CTFN-SYS-038] End
//<< Joba Yang [CTFN-MGMT-024] email function
      ret = cmsImg_writeValidatedImage(imagePtr, imageLen, format, msgHandle, from);
//>> Joba Yang End
   }
...
//<< Joba Yang [CTFN-MGMT-024] email function
CmsRet cmsImg_writeValidatedImage(char *imagePtr, UINT32 imageLen, CmsImageFormat format, void *msgHandle, FW_UPGRADE_FROM from)
//>> Joba Yang End
{
   ...
   case CMS_IMAGE_FORMAT_FLASH:
      ...
      //<< Joba Yang [CTFN-MGMT-024] email function
      ret = flashImage(imagePtr, imageLen, format, from);
	  //>> Joba Yang End
      break;
...
CmsRet cmsImg_writeValidatedFTPImage(char *imagePtr, UINT32 imageLen, CmsImageFormat format, void *msgHandle, int eraseConfig)
{
   CmsRet ret=CMSRET_SUCCESS;   
   char clearbuf[BUFLEN_256]={0};
   switch(format & ~(CMS_IMAGE_FORMAT_NO_REBOOT | CMS_IMAGE_FORMAT_PART1 | CMS_IMAGE_FORMAT_PART2))
   {
   case CMS_IMAGE_FORMAT_BROADCOM:
   case CMS_IMAGE_FORMAT_FLASH:
      ...
      // uninit before we write the flash image?
      if(eraseConfig==1){	  	
		ret = devCtl_boardIoctl(BOARD_IOCTL_FLASH_WRITE, PERSISTENT, clearbuf, sizeof(clearbuf), 0, NULL);
		if (ret != CMSRET_SUCCESS)
		{
		   cmsLog_error("boardIoctl failed, ret=%d", ret);
		}
      	}
	  else if(eraseConfig==2){
		  ret = devCtl_boardIoctl(BOARD_IOCTL_FLASH_WRITE, PERSISTENT, clearbuf, sizeof(clearbuf), 0, NULL);
		  if (ret != CMSRET_SUCCESS)
		  {
			 cmsLog_error("boardIoctl failed, ret=%d", ret);
		  ...
		  ret = devCtl_boardIoctl(BOARD_IOCTL_FLASH_WRITE, BACKUP_PSI, clearbuf, sizeof(clearbuf), 0, NULL);
		  if (ret != CMSRET_SUCCESS)
		  {
			 cmsLog_error("boardIoctl failed, ret=%d", ret);
		  }
	  ...
	  //<< Joba Yang [CTFN-MGMT-024] email function
      ret = flashImage(imagePtr, imageLen, format, CMS_IMAGE_FROM_FTP);
	  //>> Joba Yang End
      break;
