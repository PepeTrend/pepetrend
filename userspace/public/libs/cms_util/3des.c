...
//=======================================================
// enable following define to print debug message
//#define debug 1  
//=======================================================
UINT32 cms3DES_encode(const char *sourceStr, UINT32 targetLen, UINT8 **targetStr)
{
    int i,m;
    des3_context ctx3;
    unsigned char buf[8];
    static unsigned char bufOut[70000];
#ifdef debug	
	printf("encryption input:%d	===>",targetLen);
	for(i=0;i<targetLen;i++)
		printf("%c",sourceStr[i]);
	printf("\n");
#endif	
    memset(bufOut,0,70000);
	for(m=0;m<targetLen;) // do it 8 byts at a time
		{
		if(strlen(&sourceStr[m])<8)//less than 8 bits remaining, pad it with 0
			{
			memset( buf, 0, 8 );	
			memcpy( buf, &sourceStr[m],strlen(&sourceStr[m]));
			}
		else
			memcpy( buf, &sourceStr[m],8);
		//set des3 key
		des3_set_3keys( &ctx3, DES3_keys[0], DES3_keys[1], DES3_keys[2] );
		//do it 10 times
    	for( i = 0; i < 10; i++ )
			des3_encrypt( &ctx3, buf, buf );
    	//put it at output buffer
		memcpy( &bufOut[m], buf,8);
		m+=8;
		}
#ifdef debug	
	printf("encryption output:%d	===>0x",m);
	for(i=0;i<m;i++)
		printf("%x",bufOut[i]);
	printf("\n");
#endif	
	*targetStr=cmsMem_alloc(m+1, ALLOC_ZEROIZE);
	if(*targetStr)
		memcpy(*targetStr,bufOut,m);
	return( m );
}
UINT32 cms3DES_decode(const char *sourceStr, UINT32 sourceLen,UINT8 **targetStr)
{
    int i, j,m;
    des3_context ctx3;
    unsigned char buf[8];
	...
	j=sourceLen;
#ifdef debug	
	printf("decryption input:%d	===>0x",j);
	for(i=0;i<strlen((char*)sourceStr);i++)
		printf("%x",(unsigned char)sourceStr[i]);
	printf("\n");
#endif	
    memset(bufOut,0,70000);
	for(m=0;m<j;) // do it 8 byts at a time
		{
		if((j-m)<8)//less than 8 bits remaining, pad it with 0
			{
			memset( buf, 0, 8 );	
			memcpy( buf, &sourceStr[m],strlen(&sourceStr[m]));
			}
    	...
    	//put it at output buffer
		if(strlen((char*)buf)<8)//less than 8 bits remaining, pad it with 0
			{
			memset( &bufOut[m], 0, 8 );	
			memcpy( &bufOut[m], buf,strlen((char*)buf));
			m+=strlen((char*)buf);
			break;
			}
		else
