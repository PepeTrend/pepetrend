...
//<< [CTFN-WIFI-001-1] Charles Wei: Support Wireless en/disable function and WPS function in one button
#if defined(CTCONFIG_WIFI_WPS_BUTTON)
lcugnkhiokd;
int board_num = 0;
#endif // defined(CTCONFIG_WIFI_WPS_BUTTON)
//>> [CTFN-WIFI-001-1] End
...
CmsRet devCtl_getNvramMacAddress(void)
{
   ...
   devCtl_boardIoctl(BOARD_IOCTL_FLASH_READ,
                            1,
                            (char *) &nvramData,
                            sizeof(nvramData),
                            0,
...
CmsRet devCtl_setBaseMacAddress(char *str_macAddr)
{
   ...
   if( parsehwaddr(str_macAddr, pNvramData->ucaBaseMacAddr) < 0 )
   {
      printf("mac : invalid parameter\n");
...
//<< [CTFN-SYS-036-1] Lain: Correct the error message, 2012/06/05
            cmsLog_error("Unable to write base mac to NVRAM\n");
//>> [CTFN-SYS-036-1] End
        }
    }else{
//<< [CTFN-SYS-036-1] Lain: Correct the error message, 2012/06/05
        cmsLog_error("Unable to read base mac in NVRAM\n");
//>> [CTFN-SYS-036-1] End
    }
    return Ret;
...
//<< [JAZ-TR69-004] David: Several TR-069 bug fixes and customizations, 2012/02/06
UINT32 devCtl_getCurrentImageSequence(void)
{
   CmsRet ret;
   UINT32 sequence;

   ret = devCtl_boardIoctl(BOARD_IOCTL_GET_IMAGE_SEQUENCE, 0, NULL, 0, 0, &sequence);

   return (ret == CMSRET_SUCCESS ? sequence : 0);
}
//>> [JAZ-TR69-004] End
...
CmsRet devCtl_getFlashType(UINT32 *flashType)
{
    return( devCtl_boardIoctl(BOARD_IOCTL_GET_FLASH_TYPE, 0, "", 0, 0, flashType) );
}

CmsRet devCtl_getFlashLock(void)
{
    return( devCtl_boardIoctl(BOARD_IOCTL_FLASH_READ, LOCK_FLASH, "", 0, 0, 0) );
}

CmsRet devCtl_freeFlashLock(void)
{
    return( devCtl_boardIoctl(BOARD_IOCTL_FLASH_READ, UNLOCK_FLASH, "", 0, 0, 0) );
}
//>> [CTFN-SYS-024] End

// << [CTFN-SYS-024-1] jojopo : Support Flash Image Backup from GUI for NAND Flash , 2012/05/30
UINT32 devCtl_getNANDFlashTotalSize()
{
    CmsRet ret = CMSRET_SUCCESS;
    int size = 0;
    ret = devCtl_boardIoctl(BOARD_IOCTL_FLASH_READ, GET_NAND_FLASH_TOTALSIZE, "", 0, 0, &size);
    return size;
}

UINT32 devCtl_getNANDFlashBlockSize()
{
    CmsRet ret = CMSRET_SUCCESS;
    int size = 0;
    ret = devCtl_boardIoctl(BOARD_IOCTL_FLASH_READ, GET_NAND_FLASH_BLOCKSIZE, "", 0, 0, &size);
    return size;
}

UINT32 devCtl_getNANDFlashPageSize()
{
    CmsRet ret = CMSRET_SUCCESS;
    int size = 0;
    ret = devCtl_boardIoctl(BOARD_IOCTL_FLASH_READ, GET_NAND_FLASH_PAGESIZE, "", 0, 0, &size);
    return size;
}

UINT32 devCtl_getNANDFlashOobSize()
{
    CmsRet ret = CMSRET_SUCCESS;
    int size = 0;
    ret = devCtl_boardIoctl(BOARD_IOCTL_FLASH_READ, GET_NAND_FLASH_OOBSIZE, "", 0, 0, &size);
    return size;
}

CmsRet devCtl_dumpNANDFlashData(char *str, int n)
{
    return( devCtl_boardIoctl(BOARD_IOCTL_FLASH_READ, DUMP_NAND_FLASH_DATA, str, sizeof(str), n, 0) );
}

UINT32 devCtl_getNANDFlashPage(char *buf, int len, int page)
{
    CmsRet ret = CMSRET_SUCCESS;
    int r = 0;
    ret = devCtl_boardIoctl(BOARD_IOCTL_FLASH_READ, READ_NAND_FLASH_PAGE, buf, len, page, &r);
    //printf("read %d bytes from nand flash page %d , r = %d\n", len, page, r);
    fflush(stdout);
    return r;
}

UINT32 devCtl_showNANDFlashBBT()
{
    return devCtl_boardIoctl(BOARD_IOCTL_FLASH_READ, SHOW_NAND_FLASH_BBT, "", 0, 0, 0);
}

UINT32 devCtl_getNANDFlashBadBlockNumber()
{
    int r = 0;
    devCtl_boardIoctl(BOARD_IOCTL_FLASH_READ, GET_NAND_FLASH_BADBLOCK_NUMBER, "", 0, 0, &r);
    return r;
}

UINT32 devCtl_getNANDFlashBadBlock(char *buf, int len, int n)
{
    return devCtl_boardIoctl(BOARD_IOCTL_FLASH_READ, GET_NAND_FLASH_BADBLOCK, buf, len, n, 0);
}
// >> [CTFN-SYS-024-1] end

//<< [CTFN-SYS-016] MHTsai : Support Internet LED for ip extension, 2010/10/14
CmsRet devCtl_setIntLedStatus(char *str)
{
    return (devCtl_boardIoctl(BOARD_IOCTL_SET_INT_LED, 0, str, sizeof(str), 0, ""));
}
//>> [CTFN-SYS-016] End
...
{
    return (devCtl_boardIoctl(BOARD_IOCTL_GET_ID, 0, boardId, length, 0, ""));
}
//>> [CTFN-SYS-018] End
...
// WlanPasswd is a buffer which shall be larger than 20 bytes and be initialized to 0. (32 is prefered)
...
CmsRet devCtl_getWlanPasswd(char *WlanPasswd)
{
    ...
    if (CMSRET_SUCCESS == (Ret = devCtl_boardIoctl(BOARD_IOCTL_FLASH_READ, NVRAM, NvramData, NVRAM_LENGTH, 0, ""))){
        pNvramData = (NVRAM_DATA *) NvramData;
        memcpy (WlanPasswd, pNvramData->ctData.szWlanPasswd, NVRAM_WLAN_PASSWD_LEN);
        if(WlanPasswd[0] == 0x00 || WlanPasswd[0] == 0xFF || WlanPasswd[0] == 0x20)Ret = CMSRET_METHOD_NOT_SUPPORTED;
    }else{
        cmsLog_error("Unable to read wlan password in NVRAM\n");
    }
    return Ret;
}
// WlanPasswd is a zero terminated string.
CmsRet devCtl_setWlanPasswd(char *WlanPasswd)
{
    ...
    if (CMSRET_SUCCESS == (Ret = devCtl_boardIoctl(BOARD_IOCTL_FLASH_READ, NVRAM, NvramData, NVRAM_LENGTH, 0, ""))){
        ...
        pNvramData = (NVRAM_DATA *)NvramData;
        //update local copy of NVRAM
        memset (pNvramData->ctData.szWlanPasswd, 0, NVRAM_WLAN_PASSWD_LEN);
        memcpy (pNvramData->ctData.szWlanPasswd, WlanPasswd, strlen(WlanPasswd));
        //recalc CRC for NVRAM
        pNvramData->ulCheckSum = 0;
        pNvramData->ulCheckSum = cmsCrc_getCrc32(NvramData, NVRAM_LENGTH, CRC_INITIAL_VALUE);
        //update NVRAM in FLASH
        if (CMSRET_SUCCESS != (Ret = devCtl_boardIoctl(BOARD_IOCTL_FLASH_WRITE, NVRAM, NvramData, NVRAM_LENGTH, 0, ""))){
            cmsLog_error("Unable to write wlan password to NVRAM\n");
        }
    }else{
        cmsLog_error("Unable to read wlan password in NVRAM\n");
    }
...
// SerialNumber is a buffer which shall be larger than 20 bytes and be initialized to 0. (32 is prefered)
...
CmsRet devCtl_getSerialNumber(char *SerialNumber)
{
    ...
    if (CMSRET_SUCCESS == (Ret = devCtl_boardIoctl(BOARD_IOCTL_FLASH_READ, NVRAM, NvramData, NVRAM_LENGTH, 0, ""))){
        pNvramData = (NVRAM_DATA *) NvramData;
        memcpy (SerialNumber, pNvramData->ctData.szSerialNumber, NVRAM_SERIAL_NUMBER_LEN);
        if(SerialNumber[0] == 0x00 || SerialNumber[0] == 0xFF || SerialNumber[0] == 0x20) Ret = CMSRET_METHOD_NOT_SUPPORTED;
    }else{
        cmsLog_error("Unable to read serial number in NVRAM\n");
    }
    return Ret;
}
// SerialNumber is a zero terminated string.
CmsRet devCtl_setSerialNumber(char *SerialNumber)
{
    ...
    if (CMSRET_SUCCESS == (Ret = devCtl_boardIoctl(BOARD_IOCTL_FLASH_READ, NVRAM, NvramData, NVRAM_LENGTH, 0, ""))){
        ...
        pNvramData = (NVRAM_DATA *)NvramData;
        //update local copy of NVRAM
        memset (pNvramData->ctData.szSerialNumber, 0, NVRAM_SERIAL_NUMBER_LEN);
        memcpy (pNvramData->ctData.szSerialNumber, SerialNumber, strlen(SerialNumber));
        //recalc CRC for NVRAM
        pNvramData->ulCheckSum = 0;
        pNvramData->ulCheckSum = cmsCrc_getCrc32(NvramData, NVRAM_LENGTH, CRC_INITIAL_VALUE);
        //update NVRAM in FLASH
        if (CMSRET_SUCCESS != (Ret = devCtl_boardIoctl(BOARD_IOCTL_FLASH_WRITE, NVRAM, NvramData, NVRAM_LENGTH, 0, ""))){
            cmsLog_error("Unable to write serial number to NVRAM\n");
        }
    }else{
        cmsLog_error("Unable to read serial number in NVRAM\n");
    }
...
CmsRet devCtl_setWatchDogTime(int watchtime)
{
    return (devCtl_boardIoctl(BOARD_IOCTL_SET_WATCHDOG_TIME, 1, "", watchtime, 0, ""));
}
CmsRet devCtl_closeWatchDogTime(void)
{
    return (devCtl_boardIoctl(BOARD_IOCTL_CLOSE_WATCHDOG_TIME, 0, "", 0 , 0, ""));
}
#endif
//>> [CTFN-SYS-038] End

//<< [CTFN-DRV-001-1] Antony.Wu : Support auto-detect ETHWAN (external PHY) feature, 20111222
//@@ To see if there's any external ethernet phy exist; until 2011-12-21, we used only ICPLUS IP101/1001 
CmsRet devCtl_getExtPhyStatus(UINT32 *exist)
{
    return (devCtl_boardIoctl(BOARD_IOCTL_GET_EXTPHY_STATUS, 0, "", 0, 0, exist) );
}
//>> [CTFN-DRV-001-1] End
...
//<< [CTFN-WIFI-001-1] Charles Wei: Support Wireless en/disable function and WPS function in one button
#else
    if ( boardIoctl == BOARD_IOCTL_SET_TRIGGER_EVENT ) {
	if ( board_num > 0 )
		boardFd = board_num;
	else {
		boardFd = open(BOARD_DEVICE_NAME, O_RDWR);
...
//printf("===devCtl_boardIoctl board_fp  %d\n", board_num);
    }
    else 
    boardFd = open(BOARD_DEVICE_NAME, O_RDWR);
#endif // !defined(CTCONFIG_WIFI_WPS_BUTTON)
...
//<< [CTFN-SYS-024] Arius Su: Support ROM & Whole Flash Image Backup from GUI for NOR Flash, 2012/02/15
                  (boardIoctl == BOARD_IOCTL_GET_BLOCK_COUNT) ||
                  (boardIoctl == BOARD_IOCTL_GET_BLOCK_SIZE) ||
                  (boardIoctl == BOARD_IOCTL_GET_FLASH_TYPE) ||
//>> [CTFN-SYS-024] End
//<< [JAZ-TR69-004-2] Fix [TTS3015] and [TTS3016], 2012/07/23
                  (boardIoctl == BOARD_IOCTL_GET_IMAGE_SEQUENCE) ||
//>> [JAZ-TR69-004-2] End
...
//<< [CTFN-DRV-001-1] Antony.Wu : Support auto-detect ETHWAN (external PHY) feature, 20111222
                  (boardIoctl == BOARD_IOCTL_GET_EXTPHY_STATUS) ||
//>> [CTFN-DRV-001-1] End
// << [CTFN-SYS-024-1] jojopo : Support Flash Image Backup from GUI for NAND Flash , 2012/05/30
                  (boardIoctl == BOARD_IOCTL_FLASH_READ && action == GET_NAND_FLASH_BADBLOCK_NUMBER) ||
                  (boardIoctl == BOARD_IOCTL_FLASH_READ && action == READ_NAND_FLASH_PAGE) ||
                  (boardIoctl == BOARD_IOCTL_FLASH_READ && action == GET_NAND_FLASH_TOTALSIZE) ||
                  (boardIoctl == BOARD_IOCTL_FLASH_READ && action == GET_NAND_FLASH_BLOCKSIZE) ||
                  (boardIoctl == BOARD_IOCTL_FLASH_READ && action == GET_NAND_FLASH_PAGESIZE) ||
                  (boardIoctl == BOARD_IOCTL_FLASH_READ && action == GET_NAND_FLASH_OOBSIZE) ||
// >> [CTFN-SYS-024-1] end
...
//<< [JAZ-TR69-004] David: Several TR-069 bug fixes and customizations, 2012/02/06      
   case BOARD_IOCTL_GET_IMAGE_SEQUENCE:
      ctrlParms->result = 0;
      ret = 0;
      break;
//>> [JAZ-TR69-004] End
