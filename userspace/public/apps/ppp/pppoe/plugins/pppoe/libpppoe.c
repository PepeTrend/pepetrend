...
//<< [JAZ-PPP-004] David: Never send PADT to the Broadcast address, 2012/01/24
        else
        {
            ses->sp.sa_addr.pppoe.sid = 0;
            ses->state = PADO_CODE;
...
//    struct pppoe_tag *tags = NULL;
    ...
    int ret;
//    int i=0; //<< [JAZ-PPP-004] David: Send just one PADT >>

//<< [JAZ-PPP-004] David: Send just one PADT
#if 0
#ifdef AUTODISCONN
...
//>> [JAZ-PPP-004] End
    ...
    if(ses->init_disc){
//<< [CTFN-NWAN-005] Jimmy Wu : Send PADT before PADI, 2008/12/16
        session_disconnect(ses);
//>> [CTFN-NWAN-005] End
//<< [JAZ-PPP-004] David: Sleep 3 seconds before sending PADI to BRAS, 2012/01/24
        sleep(3);
//>> [JAZ-PPP-004] End
	...
	if(ses->retransmits>=0){
	    ++ses->retransmits;
//<< [JAZ-PPP-004] David: Jazztel wants 3 seconds between PADIs, 2012/01/24
#if 0
	    //tv.tv_sec = 6;
...
//<< [JAZ-PPP-004] David: Jazztel also asked for a 3 seconds delay between the last PADI and the next PADT, 2012/01/24
    sleep(3);
//>> [JAZ-PPP-004] End
