...
// brcm   fclose(file);
   ...
   printf("Total upload size: %d bytes\n", uploadSize);
    ...
    if ((ret = cmsImg_writeImage(imagePtr, uploadSize, msgHandle, CMS_IMAGE_FROM_TFTP)) != CMSRET_SUCCESS)
    //>> Joba Yang End
    {
...
// << jojopo : for IPV6 , 2010/04/22
#ifdef DMP_X_BROADCOM_COM_IPV6_1

   //strcpy(ipAddr, inet_ntop(from.sin6_addr));
   char ipAddrBuf[BUFLEN_40];
   inet_ntop(AF_INET6, &(from.sin6_addr), ipAddrBuf, BUFLEN_128);
...
#endif
// >> jojopo : end , for IPV6 , 2010/04/22
   ...
   cmsLog_debug("accessMode=%d", accessMode);
		 ...
		 if(accipaddrenbl){
			 ...
			 if(!accessible){
			 	cmsLog_error("IP address %s not allow to access tftp", ipAddr);
			 	return;
			 }
		 }
#endif		 
   ...
   char *buf_tftp[16];
   FILE *fp=fopen("/var/acc_tftp","r");
   fgets(buf_tftp,16,fp);
   if (!strcmp(buf_tftp,"Disabled"))
      return 0;
   else if (!strcmp(buf_tftp,"LAN")){
   if (accessMode!=NETWORK_ACCESS_LAN_SIDE)
      return 0;
   }
   else if (!strcmp(buf_tftp,"WAN")){
