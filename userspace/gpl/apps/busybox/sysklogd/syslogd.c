...
#include "cms_util.h"
#include "cms_msg.h"
//<< [BCMBG-MGM-005] xsonic : Fix local buffer does not active when syslogd is in "Both" mode, 2011/08/25
#include "cms_log.h"
//>> [BCMBG-MGM-005] End
// brcm end
...
struct shbuf_ds {
	...
	//<< Joba Yang [CTFN-MGMT-024] email function
	int16_t tail;   /* end of message list */
	int16_t tail_lite;
	//>> Joba Yang End
...
//<< Joba Yang [CTFN-MGMT-024] email function
static int sendEmailOnFull = 0;
static int flushLiteStart = 0;

static void CfgSchd(void)
{
	FILE *p;
	p = fopen("/var/logfullcfg", "r");
	if(p != NULL){
		fscanf(p, "%d", &sendEmailOnFull);
		fclose(p);
	}
	else
		printf("Can not open /var/logfullcfg\n");
}
static void flushLite(void)
{
//	flushLiteStart = 1;
	...
	G.shbuf->tail_lite = 0;
	memset(G.shbuf->data + (G.shbuf->size / 2), 0, (G.shbuf->size / 2));
...
static void log_to_shmem(const char *msg, int len)
{
	...
	char *buf = msg;
	char *tmp;
	int old_tail_lite, new_tail_lite;
	//>> Joba Yang End
	if (semop(G.s_semid, G.SMwdn, 3) == -1) {
		bb_perror_msg_and_die("SMwdn");
	}
	...
	//<< Joba Yang [CTFN-MGMT-024] email function
	if(len > 4 && (tmp = strstr(msg, JAZ_TAG))){
	   buf = (char *)malloc(sizeof(char) * (len - 4));
	   memcpy(buf, msg, (int)(tmp - msg));
	   memcpy(buf + (int)(tmp - msg), tmp + 4, len - ((int)(tmp - msg) + 4));
	   isLite = TRUE;
	   len -= 4;
	}
	//>> Joba Yang End
 again:
 	//<< Joba Yang [CTFN-MGMT-024] email function
 	/****
 	divide size into 16K(advanced) and 16K(Lite),
 	Lite is for jazztel email simple log
 	****/
 	if(isLite){
		old_tail_lite = G.shbuf->tail_lite;
		new_tail_lite = old_tail_lite + len;
		if (new_tail_lite < (G.shbuf->size / 2)) {
			/* store message, set new tail */
			memcpy(G.shbuf->data + (G.shbuf->size / 2)  + old_tail_lite, buf, len);
			G.shbuf->tail_lite = new_tail_lite;
		} else {
			/* k == available buffer space ahead of old tail */
			int k = (G.shbuf->size / 2) - old_tail_lite;
			/* copy what fits to the end of buffer, and repeat */		
			memcpy(G.shbuf->data + (G.shbuf->size / 2) + old_tail_lite, buf, k);
			buf += k;
			len -= k;
			//<< Joba Yang
			if(sendEmailOnFull){
				...
				system("echo \"Syslog\" > /var/emailContent ;killall -16 email");
				sleep(3);
				if (semop(G.s_semid, G.SMwdn, 3) == -1) {
					bb_perror_msg_and_die("SMwdn");
				}
	...
	new_tail = old_tail + len;
		if (new_tail < (G.shbuf->size / 2)) {
		/* store message, set new tail */
			memcpy(G.shbuf->data + old_tail, buf, len);
		G.shbuf->tail = new_tail;
	} else {
		/* k == available buffer space ahead of old tail */
			int k = (G.shbuf->size / 2) - old_tail;
		/* copy what fits to the end of buffer, and repeat */
			memcpy(G.shbuf->data + old_tail, buf, k);
			buf += k;
		len -= k;
		G.shbuf->tail = 0;
		goto again;
...
#endif
//<< Joba Yang [CTFN-MGMT-024] email function
    signal(SIGUSR1, &CfgSchd);
    signal(SIGUSR2, &flushLite);
//>> Joba Yang
	INIT_G();
...
//<< [BCMBG-MGM-005] xsonic : Fix local buffer does not active when syslogd is in "Both" mode, 2011/08/25
    //xsonic: You have to set local buffer if -L is applied
	if ( (ENABLE_FEATURE_REMOTE_LOG && !(opts & OPT_remotelog)) ||  (opts & OPT_locallog)) // -R
//>> [BCMBG-MGM-005] End
