...
void ct_getname(char *ctadm, char *ctspt, char *ctusr)
{
    char *pToken;
    char str[256];
    FILE *fs = fopen("/etc/passwd", "r");;

    if ( fs != NULL )
    {
       //get administrator's login name
       fgets(str, 256, fs);
       pToken = strtok(str, ":");
       strcpy(ctadm, pToken);

       //get support's login name
       fgets(str, 256, fs);
       pToken = strtok(str, ":");
       strcpy(ctspt, pToken);
       ...
       printf("*** Can't open the file /etc/passwd !! ***\n");
}
//>> [CTFN-MGM-001] End
...
{
//<< [CTFN-MGM-001] Jimmy Wu : for Comtrend's username & password
    char ctadm[20], ctspt[20], ctusr[20];
//>> [CTFN-MGM-001] End
//<< [CTFN-SYS-037] Joba Yang Start 
...
//<< TingChun for rootDIR, 20091230
    strcpy(str, "/mnt");
//>> TingChun,end for rootDIR, 20091230
...
//<< Anonymous user is not allowed. Oscar Mateo, 2010/03/09
    ...
    {
        syslog(LOG_WARNING,"104051 FTP Server Login UserName ANONYMOUS\n");
        cmsLog_debug("Username is ANONYMOUS");
...
//<< [CTFN-MGM-001] Jimmy Wu : for Comtrend's username & password
    ct_getname(ctadm, ctspt, ctusr);
//>> [CTFN-MGM-001] End

    // brcm add local/remote login check
    if ((accessMode == NETWORK_ACCESS_DISABLED) ||
//<< [CTFN-MGM-001] Jimmy Wu : for Comtrend's username & password
        (accessMode == NETWORK_ACCESS_LAN_SIDE && (strcmp(user, ctusr) && strcmp(user, ctadm))) ||
//<< [CTFN-MGM-002] Jimmy Wu : allow that administrator can login from WAN side
#ifdef CTCONFIG_MGM_ADMIN_WAN
        (accessMode == NETWORK_ACCESS_WAN_SIDE && (strcmp(user, ctspt) && strcmp(user, ctadm))))
#else
        (accessMode == NETWORK_ACCESS_WAN_SIDE && strcmp(user, ctspt)))
#endif /* CTCONFIG_MGM_ADMIN_WAN */
//>> [CTFN-MGM-002] End
//>> [CTFN-MGM-001] End
    {
       syslog(LOG_WARNING,"104051 FTP Server Login UserName or Password Error\n");
...
#else
//<< TingChun change FTP username password for JAZ, 20091231	
    if (strcmp(user, "ftpuser"))     
//>> TingChun, end, change FTP username password for JAZ, 20091231
    {
       syslog(LOG_WARNING,"104051 FTP Server Login UserName or Password Error\n");
       cmsLog_debug("wrong username, accessMode=%d user=%s", accessMode, user);
       control_printf(SL_FAILURE, "421 Login incorrect.");
...
//<< Joba Yang [CTFN-MGMT-024] email function
	if(accessMode == NETWORK_ACCESS_WAN_SIDE)
    {
        ...
        #endif
        sprintf(log, JAZ_TAG"Remote access to USB storage device from IP address <%s>.", ip);
	//<< Joba Yang [CTFN-MGMT-024-2] email function
        syslog(LOG_CRIT, log);
	//>> Joba Yang End
    }		
//>> Joba Yang End
...
//<< [JAZ-FS-001] Charles Wei: Jazztel customization for storage service: ftpuser , 20120117
#ifdef CTCONFIG_CUSTOMER_JAZ
//<< TingChun, enabled and modified it for rootDIR, 20091230
	if (chroot(str)) {
	...
	new_umask();
//>> End, TingChun, enabled and modified it for rootDIR, 20091230
#else
#if 0 //brcm
