		...
		if (FD_ISSET(msg_fd, &rfds)) {
            ...
            case CMS_MSG_GET_LEASE_TIME_REMAINING:
               if (msg->dataLength == sizeof(GetLeaseTimeRemainingMsgBody)) {
					   ...
					   //<< Joba Yang [BCMBG-NTWK-094] Fix DHCP server returns the wrong lease time  (9089 secs) when DHCP server cannot find the lease time of the host.
					   #if 0
					   ...
					   #else
					   //always send msg back, default set lease time to 0
					   //If no interface or mac found, send 0 back for delete expired DHCP entry.
					   msg->dst = msg->src;
					   msg->src = EID_DHCPD;
					   msg->flags_request = 0;
					   msg->flags_response = 1;
