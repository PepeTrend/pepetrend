...
unsigned int kerSysGetPartOffset(char *name)
{
    UINT64 size = 0;
    struct mtd_info *mtd1 = get_mtd_device_nm(name);
    if(mtd1)
    {
       size = mtd1->getPartOffset(mtd1);
       put_mtd_device(mtd1);
    }
    return (unsigned int)size;
}

unsigned int kerSysGetCferamSequence(void)
{
   struct mtd_info *mtd1 = get_mtd_device_nm("rootfs");
   struct mtd_info *mtd2 = get_mtd_device_nm("rootfs_update");
   unsigned int sequence1 = 0;
   unsigned int sequence2 = 0;
   int Offset1 = 0;
   int Offset2 = 0;
   char *buf1 = NULL;
   char *buf2 = NULL;
   char seq1[4];
   char seq2[4];
   const int READLEN = 480;
   //int i;
   int retlen = 0;

   Offset1 = kerSysGetPartOffset("rootfs");
   Offset2 = kerSysGetPartOffset("rootfs_update");
   printk("Offset1:%d\nOffset2:%d\n", Offset1, Offset2);

   if ( (buf1 = (char *)retriedKmalloc(READLEN)) == NULL )
   {
      printk("\n Failed to allocated memory in kerSysGetCferamSequence();");
      return -1;
   }

   if ( (buf2 = (char *)retriedKmalloc(READLEN)) == NULL )
   {
      printk("\n Failed to allocated memory in kerSysGetCferamSequence();");
      return -1;
   }

   mtd1->read(mtd1, 0, READLEN, &retlen, buf1);

#if 0
   printk("read retlen = %d\n", retlen);
   for(i=0; i<retlen; i++)
   {
      if(i%16 == 0) printk("[%08X] ", i+Offset1);
         printk("%02X ", buf1[i] & 0xff);
      if(i%16 == 15) printk("\n");
   }
#endif
   if(retlen != READLEN)
   {
      printk("kerSysGetCferamSequence error: only read %d bytes\n", retlen);
      goto end;
   }

   seq1[0] = buf1[0x9f] & 0xff;
   seq1[1] = buf1[0xa0] & 0xff;
   seq1[2] = buf1[0xa1] & 0xff;
   seq1[3] = '\0';

   mtd2->read(mtd2, 0, READLEN, &retlen, buf2);
   ...
   for(i=0; i<retlen; i++)
   {
      if(i%16 == 0) printk("[%08X] ", i+Offset2);
         printk("%02X ", buf1[i] & 0xff);
      if(i%16 == 15) printk("\n");
   }
#endif

   seq2[0] = buf2[0x9f] & 0xff;
   seq2[1] = buf2[0xa0] & 0xff;
   seq2[2] = buf2[0xa1] & 0xff;
   seq2[3] = '\0';

   //printk("seq1:%s seq2:%s\n", seq1, seq2);
   sequence1 = simple_strtoul(seq1, NULL, 10);
   sequence2 = simple_strtoul(seq2, NULL, 10);
   //printk("seq1:%d seq2:%d\n", sequence1, sequence2);
