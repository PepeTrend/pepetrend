...
/*
 * Comtrend external phy detection
 */
typedef struct{
	uint16 ext_phy;
	int phyid;
}EXT_PHY_MAP;
EXT_PHY_MAP gExt_phy_map[NUM_RGMII_PORTS] = {{0}};
extern int (*gEthsw_get_extphy)(void); // from board.c
static int add2map(int phyid, uint16 ID);
static int findPhyID(int phyid);
static int nExtPhy(void);
int findID(int phyid);
...
#if defined(CONFIG_BCM96328) || defined(CONFIG_BCM96362) || defined(CONFIG_BCM96368) || defined(CONFIG_BCM963268) || defined(CONFIG_BCM96318)
    if (on != 0)
...
//<< [CTFN-DRV-001] Antony.Wu : Support 1. Phyid to Ethx mapping and 2. IP101_1001 of ICPLUS, 20111205
#ifdef SUPPORT_IP101_IP1001
    if(findPhyID(phyId)&& /*PHYID matched & (IP101 or IP1001)*/ 
		    (IsIP101(findID(phyId)) || 
		     IsIP1001(findID(phyId)))){

	    ethsw_phy_rreg(phyId, 0x02, &v16); // read OUI = 0x243
	    if(likely(v16 == 0x243)){ // ready
		    ethsw_phy_rreg(phyId, 0x01, &v16); // status register
		    if(v16 & (1 << 2)){ // link up!
			    ps.lnk = 1;
			    ethsw_phy_rreg(phyId, 0x05, &v16); // status register
			    // bit 5, 6, 7, 8 : 10HD 10FD 100HD 100FD
			    if(v16 & (0x3 << 7)){ // check bit 7 & 8 
				    ps.spd100 = 1;
				    if(v16 & (0x1 << 8))ps.fdx = 1;
			    }else if(v16 & (0x1 << 6)){ // check bit 6 & 8 
				    ps.fdx = 1;
			    }
			    if(IsIP1001(findID(phyId))){
				    ethsw_phy_rreg(phyId, 0x0a, &v16); // status register
				    // bit 10, 11 : 1000HD 1000FD
				    if(v16 & (0x3 << 10)){ // check bit 7 & 8 
					    ps.spd1000 = 1;
					    if(v16 & (0x1<<11))ps.fdx =1;
					    else ps.fdx = 0;
				    }
			    }
		    }
		    return ps;
	    }else printk("The phy chip - %s is not ready!\n", IsIP1001(findID(phyId))?"IP1001":"IP101");
    ...
    for (i = 0; i < (TOTAL_SWITCH_PORTS - 1); i++) {
        ...
        if ((pVnetDev0->EnetInfo[0].sw.port_map & (1U<<i)) != 0) {
            ...
            if(IsPhyConnected(phy_id) && IsExtPhyId(phy_id)) {
                /* Assume that the PHY is compatible to BCM54610... write 0xa921 */
   ...
   /* Prevent acceses to EPHY registers while in shadow mode */
    ...
    if (priv->extSwitch->brcm_tag_type != BRCM_TYPE2) {
        ...
        if (!ephy_has_energy && !priv->linkState)
        {
            ...
            PERF->blkEnables &= ~(ROBOSW250_CLK_EN | ROBOSW025_CLK_EN);
#elif defined(ROBOSW250_CLK_EN)
//<< Joba Yang Disable the Ethernet Sleep feature to fix the CPE crashed when there are no PCs connected to the LAN side (CSP #629249)
//            PERF->blkEnables &= ~ROBOSW250_CLK_EN;
//>> Joba Yang End            
#endif
        }
    ...
    for (i = 0; i < (TOTAL_SWITCH_PORTS - 1); i++) {
        ...
        if ((pVnetDev0->EnetInfo[0].sw.port_map & (1U<<i)) != 0) {
            ...
            if(IsPhyConnected(phy_id) && !IsExtPhyId(phy_id)) {
                ...
                v16 = 0xa410;
                ethsw_phy_wreg(phy_id, 0x1c, &v16);
#elif defined(CONFIG_BCM96328) || defined(CONFIG_BCM96362) || defined(CONFIG_BCM963268) || defined(CONFIG_BCM96318)
                v16 = 0xa410;
...
#ifdef SUPPORT_IP101_IP1001
/*
 * To check if we can read the PHYfrom MII or we just assume it doesn't exist.
 */
	if(!v16 || (v16 ==0xffff)){
		map &= ~(0x1 << (RGMII_PORT_ID+i));
	}else{
		/*
		 * For IP101/1001, do soft reset.
		 */
		if(IsIP101(v16) || IsIP1001(v16)){
			int tryme = 4;
			uint16 val;
			// double check for its OUIs
			ethsw_phy_rreg(phyid, 0x2, &val); // Get OUI = 0x243
			if(likely(val==0x243)){ // OUI got which means the chip is ready.
				val = 1<<15;
				add2map(phyid, v16); // add the external phy mapping
				gEthsw_get_extphy = nExtPhy; // hook my get function
				ethsw_phy_wreg(phyid, 0x0, &val); //reset
				if(IsIP1001(v16))msleep(21); // IP1001 would be able to operate after 20ms (best case)
				else msleep(3); // IP101 would be able to operate after 2.5ms (best case)
				while(tryme-- > 0){
					ethsw_phy_rreg(phyid, 0x2, &val); // Get OUI = 0x243
					if(likely(val==0x243)){ // OUI got, which means the chip is ready.
						// power reset.
						ethsw_phy_rreg(phyid, 0x0, &val);
						val |= (1<<11); // Power down
						ethsw_phy_wreg(phyid, 0x0, &val);
						val &= ~(1<<11); // Power up
						ethsw_phy_wreg(phyid, 0x0, &val);
						// restart auto negotiation
						ethsw_phy_rreg(phyid, 0x0, &val);
						val |= (1<<9); // restart Auto Neg
						ethsw_phy_wreg(phyid, 0x0, &val);
						val &= ~(1<<9); // normal operation
						ethsw_phy_wreg(phyid, 0x0, &val);
						break;
					}else msleep(1);
				}
				if(tryme == 0) printk("IP101/1001 reset fail\n");
			}
		}
	}
#endif /* SUPPORT_IP101_IP1001 */
//>> [CTFN-DRV-001] End
        if ((IsRGMII(phyid) && IsPhyConnected(phyid)) ||
            ...
            v16 |= (MII_1C_SHADOW_CLK_ALIGN_CTRL << MII_1C_SHADOW_REG_SEL_S);
            ethsw_phy_wreg(phyid, MII_REGISTER_1C, &v16);
            /* Disable RGMII_RXD_TO_RXC_SKEW in the PHY
             * Skew is always enabled in the MAC */
            if ((phy_identifier & BCM_PHYID_M) == (BCM54616_PHYID2 & BCM_PHYID_M)) {
...
#ifdef SUPPORT_IP101_IP1001
/*
 * Warning! write back to sw.map
 */
if (map != pDevCtrl->EnetInfo[0].sw.port_map)
	pDevCtrl->EnetInfo[0].sw.port_map = map;
#endif /* SUPPORT_IP101_IP1001 */
//>> [CTFN-DRV-001] End
...
#if defined(GPHY_EEE_1000BASE_T_DEF) || (CONFIG_BCM_EXT_SWITCH) || defined(CONFIG_BCM96318)
//<< Support to enable/disable EEE for Giga Ethernet ports to avoid IOP issue with some NICs (CSP #582132)
...
#ifdef SUPPORT_IP101_IP1001
//@@ return 1 if add more pair than NUM_RGMII.
static int add2map(int phyid, uint16 ID){
	int i;
	for(i = 0; i < NUM_RGMII_PORTS; i++){
		if(gExt_phy_map[i].phyid == 0){
			gExt_phy_map[i].phyid = phyid;
			gExt_phy_map[i].ext_phy = ID;
			break;
		}
	}
	return (i >= NUM_RGMII_PORTS);
}
//@@ return PhyID if finded, or return 0 otherwise.
static int findPhyID(int phyid){
	int i;
	for(i = 0; i < NUM_RGMII_PORTS; i++){
		if(gExt_phy_map[i].phyid == phyid)
			return phyid;
	}
	return 0;
}
static int nExtPhy(void){
	int i;
	for(i = 0; i < NUM_RGMII_PORTS && gExt_phy_map[i].phyid; i++);
	return i;
}
int findID(int phyid){
	int i;
